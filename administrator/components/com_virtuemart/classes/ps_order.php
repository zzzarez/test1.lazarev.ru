...
//            $hlog=fopen('/var/www/shop.lazarev.ru/insertdownload.log','a+');
//            fwrite($hlog,"\r\n inputvalue of insert_downloads for prod    date=".date('d-m-Y H:i:s')."  downloadid=".$download_id. "  orderid=".(int) $d['order_id']." userid=".(int) $d['user_id']);
//
//            fclose($hlog);
	...
	function download_request(&$d) {
        ...
        $download_id=str_replace(' ','',$download_id); //fix!!!
        $download_id =trim($download_id);//fix!!!
		...
		if (!$download_id) {
                    ...
                    $hlog=fopen('/var/www/shop.lazarev.ru/did.log','a+');
                    fwrite($hlog,"\r\n !download_id              date=".date('d-m-Y H:i:s')."ordid=".$db->f("order_id")." download_id=".$download_id." max=".$download_max." end_date=".date('Y-m-d H:i:s',$end_date));
                    fclose($hlog);
			//$vmLogger->err( $VM_LANG->_('PHPSHOP_DOWNLOADS_ERR_INV',false) );
//            $vmLogger->err( "Извините, превышено количество скачиваний или вышло время для скачивания. Обратитесь в службу тех поддержки shop@lazarev.ru." );
            $vmLogger->err( " " );
			return false;
		...
		elseif ($download_max=="0") {
            ...
            $hlog=fopen('/var/www/shop.lazarev.ru/did.log','a+');
            fwrite($hlog,"\r\n download_max=0            date=".date('d-m-Y H:i:s')."ordid=".$db->f("order_id")." download_id=".$download_id." max=".$download_max." end_date=".date('Y-m-d H:i:s',$end_date));
            fclose($hlog);
		...
		elseif ($end_date!="0" && $zeit > $end_date) {
            $hlog=fopen('/var/www/shop.lazarev.ru/did.log','a+');
            fwrite($hlog,"\r\n end_date!=0&seit>endtate  date=".date('d-m-Y H:i:s')."ordid=".$db->f("order_id")." download_id=".$download_id." max=".$download_max." end_date=".date('Y-m-d H:i:s',$end_date));
            fclose($hlog);
		...
		if ( @file_exists( $datei ) ){
            $hlog=fopen('/var/www/shop.lazarev.ru/file_download.log','a+');
            fwrite($hlog,"\r\n http_rangeRequest - start   date=".date('d-m-Y H:i:s:u')."file=".$datei." download_id=".$download_id);
            fclose($hlog);
...
//            $hlog=fopen('/var/www/shop.lazarev.ru/file_download.log','a+');
//            fwrite($hlog,"\r\n http_rangeRequest - end   date=".date('d-m-Y H:i:s:u')."file=".$datei." download_id=".$download_id . "range_request=". implode(", ", $range_request));
//            fclose($hlog);
		} else {
			$download_count = false;
		}
...
//                $hlog=fopen('/var/www/shop.lazarev.ru/file_download.log','a+');
//                fwrite($hlog,"\r\n getRemoteFile - start   date=".date('d-m-Y H:i:s:u')."file=".$datei." download_id=".$download_id);
//                fclose($hlog);
...
//        $hlog=fopen('/var/www/shop.lazarev.ru/file_download.log','a+');
//        fwrite($hlog,"\r\n sendFile    date=".date('d-m-Y H:i:s:u')."file=".$datei." download_id=".$download_id."mime".$mime_type);
//        fclose($hlog);
	...
	function list_order($order_status='A', $secure=0 ) {
		...
		$num_rows = $db->f('num_rows');
                ...
                {
                  echo "<div align=\"right\"><button class=\"radgrad enter_button2\" onclick='javascript:document.location.href=\"/index.php?page=account.index&option=com_virtuemart&Itemid=1&typel=orders&show_all\"'>Показать все заказы</button></div>";
                }
                else
                {
                  echo "<div align=\"right\"><button class=\"radgrad enter_button2\" onclick='javascript:document.location.href=\"/index.php?page=account.index&option=com_virtuemart&Itemid=1&typel=orders\"'>Показать оплаченные заказы</button></div>";
                }
		if( $num_rows == 0 ) {
...
//			$listObj->addCell( $tmp_cell );
            ...
            $tmp_cell = vmFormatDate($db->f("cdate"), "%d. %B %Y");
            $listObj->addCell( $tmp_cell );

            $tmp_cell = sprintf("%08d", $db->f("order_id"));
            $listObj->addCell( $tmp_cell );

            $tmp_cell = $CURRENCY_DISPLAY->getFullValue($db->f("order_total"), '', $db->f('order_currency'));
            $listObj->addCell( $tmp_cell );

            //$listObj->addCell( "<span style='color: #008000'>оплачен</span>" );
            $listObj->addCell($order_status);

            $tmp_cell = "<a href=\"". $sess->url( $mm_action_url."index.php?page=account.order_details&order_id=".$db->f("order_id") )."\">\n";
			//$tmp_cell .= "<img src=\"".IMAGEURL."ps_image/goto.png\" height=\"32\" width=\"32\" align=\"middle\" border=\"0\" alt=\"".$VM_LANG->_('PHPSHOP_ORDER_LINK')."\" />&nbsp;</a><br />";
            $tmp_cell .= "подробнее";
			$listObj->addCell( $tmp_cell );
...
//            $listObj->addCell( $tmp_cell );
        ...
        echo "<br><input class='enter_button' type='button' value='Вернуться' onclick='javascript:document.location.href=\"/index.php?page=account.index&option=com_virtuemart&Itemid=1\"'></div>";
	}
