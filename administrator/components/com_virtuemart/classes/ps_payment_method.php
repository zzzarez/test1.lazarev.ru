	...
	*/
	...
	function list_payment_radio($selector, $payment_method_id, $horiz) {
		...
		while ($db->next_record()) {
            ...
            ///fix!!!
            //echo "-".PayPal (new API)."-";
            if($db->f("payment_method_name")!="PayPal (new API)")///fix!!!
            {///fix!!!
            echo '<div class="paypal_radio">';
			echo "<input style='margin-right:6px;'  type=\"radio\" name=\"payment_method_id\" id=\"".$db->f("payment_method_name")."\" value=\"".$db->f("payment_method_id")."\" ";
			if( $selector == "' OR enable_processor='Y" ) {
				echo "onchange=\"javascript: changeCreditCardList();\" ";
			}
            if($db->f("payment_method_id")==20)
            {
			...
			$discount  = $ps_checkout->get_payment_discount( $db->f("payment_method_id") );
                echo '</div><div class="paypal_sel" id="psdiv2"><div class="paypal_img"><img src="/images/ps1.png"></div><div class="paypal_txt">';
            echo $db->f("payment_method_name")."</div></div>";
