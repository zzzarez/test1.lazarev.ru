...
class vm_ps_userfield extends vmAbstractObject {
	...
	function listUserFields( $rowFields, $skipFields=array(), $db = null, $startForm = true ) {
		...
		if( $startForm ) {
            ...
            //fix!!!//echo '<form action="'. $mm_action_url .basename($_SERVER['REQUEST_URI']) .'" method="post" name="adminForm">';
		}
		echo '
		<div style="width:93%;margin-top: -18px;">';
			
		if( !empty( $required_fields ))  {
            if($_GET['typel']!='mycab')
            {
			echo '<div style="color:#495042;padding:5px;text-align:center;"><h2>Регистрация нового пользователя в магазине shop.lazarev.ru</h2><strong> <span style="font-size: 20px; color: red;">*</span> обязательно для заполнения<br><br></strong></div>';
            }
            else{
                //echo '<div style="color:#495042;padding:5px;text-align:center;"><h2>Личные данные</h2><strong> <span style="font-size: 20px; color: red;">*</span> обязательно для заполнения<br><br></strong></div>';
			...
			if( $key[0] == '_') {
	   		...
	   		if( $VM_LANG->exists($key) ) {
	   			$field->title = $VM_LANG->_($key);
                        ...
                        if ($_GET['typel'] == "mycab" && $field->name == 'password')
                        {
                            echo '<div id="passwords" class="maycab_passwords">';
                        }
	   		...
	   		if( $field->type == 'delimiter') {
	   			...
	   			//echo '<fieldset>';//!!!fix
                   if($field->title!='Регистрация'){
				        echo '<legend class="sectiontableheader">'.$field->title.'</legend>';
                   }
	   			$delimiter++;
	   			continue;
	   		}
	   		...
	   		echo '">';
               $h2='<label for="'.$field->name.'_field">'.$field->title.'</label>';
               if(($_GET['typel']=="mycab")&&($field->name=="email"))
                    $h2="";
               if(($_GET['typel']=="mycab")&&($field->name=="password"))
                   $h2='<label for="'.$field->name.'_field">Новый пароль</label>';
               echo $h2;

	        if( isset( $required_fields[$field->name] )) {
                //if($_GET['typel']!='mycab')
                if($h2!="")
	        	echo '<strong style="font-size: 20px;color: red;"> * </strong>';
	        }
	      	 ...
	      	 */
	   		...
	   		switch( $field->name ) {
	   			...
	   			case 'state':
				    ...
				    echo "</noscript>\n";
                    //echo '<input type="hidden" id="state_field" name="state_field" value="ru" class="inputbox" />';
	   				break;
				case 'agreed':
					echo '<input type="checkbox" id="agreed_field" name="agreed" value="1" class="inputbox" />';
					break;
				case 'password':
				case 'password2': //onblur="document.getElementById(\'password_span\').innerHTML=\'\'"
                   $jscript=' onfocus="document.getElementById(\'password_span\').innerHTML=\'<br>Пожалуйста, введите пароль в оба поля для избежания опечаток. <br>Длина пароля от 6 до 10 символов. <br>Допустимые символы - цифры и буквы латинского алфавита.<br><br>\';"';
					echo '<input type="password" id="'.$field->name.'_field" name="'.$field->name.'" size="30"  '.$jscript.' class="inputbox" />'.
                        " <span id='password_span' class='help_spans'> </span>\n";
		   			break;
		   			//!!!fix  ;if(document.getElementById(\'email_ticker\')) document.getElementById(\'email_ticker\').remove()
                   case 'email':
//                       echo '
//                       <script>
...
//  document.getElementById(\'email_span\').innerHTML=\'\';
                       $jscript='onchange="getEmail();document.getElementById(\'username_field\').value=document.getElementById(\'email_field\').value;"'.
                                ' onblur="document.getElementById(\'username_field\').value=document.getElementById(\'email_field\').value;   "'.
                                'onfocus="document.getElementById(\'email_span\').innerHTML=\'<br>Пожалуйста, введите Ваш адрес электронной почты<br> 1. E-mail будет использоваться для входа в магазин<br> 2. На данный e-mail будет отправлен Ваш заказ<br> 3. E-mail может быть использован для связи с Вами <br><br> \';'.
                                'if(document.getElementById(\'email_ticker\')) document.getElementById(\'email_ticker\').remove();"';
                       $hide="";
                       if($_GET['typel']=='mycab')
                       {
                       ...
                       echo '<input type="text" '.$hide.' id="'.$field->name.'_field" name="'.$field->name.'" size="30" class="inputbox" '.$jscript.'  value="'. ($db->sf($field->name, true, false)?$db->sf($field->name, true, false):'').'"/>'.
                           " <span '.$hide.' id='email_span' class='help_spans'> </span>\n";
                       break;

                   case 'username':                                                                         //onchange="getUsername();"
                       echo '<input type="text" id="'.$field->name.'_field" name="'.$field->name.'" size="30" onchange="getUsername();" maxlength="25"  class="inputbox"  value="'. ($db->sf($field->name, true, false)?$db->sf($field->name, true, false):'').'"/>';
                       break;
                    //fix!!!
	   			default:
	   				...
	   				switch( $field->type ) {
                           ...
                           {//onblur="document.getElementById(\'fn_span\').innerHTML=\'\'"
                               $scrfn='onfocus="document.getElementById(\'fn_span\').innerHTML=\'<br>Как к Вам обращаться? <br><br>\';"';
                               $spfn=" <span id='fn_span' class='help_spans'> </span>\n";
                           }

                           $maxlength = $field->maxlength ? 'maxlength="'.$field->maxlength.'"' : '';
					        echo '<input '.$scrfn.' type="text" id="'.$field->name.'_field" name="'.$field->name.'" size="'.$field->size.'" value="'. ($db->sf($field->name, true, false)?$db->sf($field->name, true, false):'') .'" class="inputbox" '.$maxlength . $readonly . ' />'."\n";
                            echo $spfn;
				   			break;
	   		...
	   		echo '<br /></div>
				      <br style="clear:both;" />';
              if ($_GET['typel'] == "mycab" && $field->name == 'password2')
              {
                  echo '</div><!-- passwords !-->';
              }
	   }
...
//            }';
        ...
        if( isset($required_fields['password']) || $page == 'account.billing') {
			if($page == 'account.billing' || $page == 'checkout.index' || $page == 'shop.registration') {
                        if($page == 'account.billing') echo '
              if (button_changepass == 1) {';
                echo '
