...
if ($cart["idx"] == 0) {
	...
	for ($i=0;$i<$cart["idx"];$i++) {
		...
		//$action_url = $mm_action_url.basename($_SERVER['PHP_SELF']); fix!!!
   ...
   <div class="img_rfr"> <input type="image" name="update" title="'. $VM_LANG->_('PHPSHOP_CART_UPDATE') .'" src="'. VM_THEMEURL .'images/vmcart_refresh.png" alt="'. $VM_LANG->_('PHPSHOP_UPDATE') .'" align="middle" />
   </div>
  </form>';
    ...
    <input type="hidden" name="description" value="'. $cart[$i]["description"].'" />
  	<div class="img_del"><input type="image" name="delete" title="'. $VM_LANG->_('PHPSHOP_CART_DELETE') .'" src="'. VM_THEMEURL .'images/vmcart_delete.png" alt="'. $VM_LANG->_('PHPSHOP_CART_DELETE') .'" align="middle" />
  	</div>
  </form>';
