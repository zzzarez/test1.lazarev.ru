...
if($_POST["TTT"]=="TTT")
{
    ...
    {//totalprice_hlp
        ...
        $onpay = new ps_onpay();
        $url=$onpay->get_onpay_url($checkoutData['order_id'],$total_price,$vmuser->email);

        //$outurl="http://secure.onpay.ru/pay/lshop?f=10&pay_mode=fix&pay_for=".$checkoutData['order_id']."&price=".$total_price."&currency=RUR&convert=yes&md5=43a666fdc718672cfcd356dd2e3ac34e&user_email=zarez@yandex.ru&url_success=http://shop.lazarev.ru/";
        vmRedirect($url);
    }
...
if ($cart["idx"] > 0) {
    ...
    if (!empty($auth['user_id'])) {
	    //echo '<h1>'. $VM_LANG->_('PHPSHOP_CHECKOUT_TITLE') .'</h1>';
    }
    else
    {
        echo '<div class="header_login"><h1>Добро пожаловать в интернет-магазин Лазарева С.Н.</h1></div>';
        echo '<div class="yes_word"><h2>Вход на сайт</h2></div><div class="no_word"><h2>Я новый покупатель</h2></div>';
    }
	...
	$basket_html .= '<form action="'. $sess->url( SECUREURL."index.php?page=".$next_page."&checkout_last_step=".$current_stage) .'" method="post" name="adminForm">
        ...
        $basket_html2='<form action="'. $sess->url( SECUREURL."index.php?page=".$next_page."&checkout_last_step=".$current_stage) .'" method="post" name="adminForm">
		<input type="hidden" name="option" value="com_virtuemart" />
		<input type="hidden" name="Itemid" value="'. $Itemid .'" />
		<input type="hidden" name="user_id" value="'. $auth['user_id'] .'" />
		<input type="hidden" name="page" value="'. $next_page .'" />
        ...
        <input type="hidden" id="TTT" name="TTT" value="TTT" />
        <input type="hidden" name="checkout_last_step" value="'. $current_stage .'" />';

		$theme->set( 'basket_html', $basket_html );
        $theme->set( 'basket_html_s', $basket_html_s );
        $theme->set( 'basket_html2', $basket_html2 );
    ...
    <br /><?php 
        ...
        if( !in_array('CHECK_OUT_GET_FINAL_CONFIRMATION', $checkout_steps[$current_stage]) ) {
         	?>
                <div align="center">
                <input type="submit" class="enter_button" style="width: 153px;font-size: 18px;" name="formSubmit" value="<?php echo "Продолжить ";//fix!!!$VM_LANG->_('PHPSHOP_CHECKOUT_NEXT');?> &gt;&gt;" />
                </div>
