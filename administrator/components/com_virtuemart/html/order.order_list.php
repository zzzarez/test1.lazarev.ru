...
$list  = "SELECT DISTINCT #__{vm}_orders.order_id,order_status, #__{vm}_orders.cdate,#__{vm}_orders.mdate,order_total,order_currency,#__{vm}_orders.user_id,";
...
$columns = Array(  "#" => "width=\"20\"", 
                    ...
                    $VM_LANG->_('PHPSHOP_ORDER_PRODUCT_LINK') => '',
...
while ($db->next_record()) { 
    ...
    $link_url = $_SERVER['PHP_SELF']."?page=order.link_generate&order_id=".$db->f("order_id");
    $link_url = $sess->url($link_url);
    // link to the product
    $listObj->addCell("<button onclick=\"javascript:void window.open('$link_url', 'link');\">".$VM_LANG->_('PHPSHOP_ORDER_GENERATE_LINK')."</button>");
