...
if ($cart["idx"] == 0) {
	...
	do
	{
		...
		$amount = 1;//fix!!!  $amount +=$cart[$i]["quantity"];
        $am++;
		...
		$subtotal = round( $price["product_price"], 2 ) * 1;//fix!!! * $cart[$i]["quantity"];
		$total += $subtotal;
        ...
        $minicart[$ci]['product_id'] = $product_id;
		if(@$_SESSION['vmCartDirection']) {
			$i++;
		}
		else {
...
//	}
    //fix!!!
    $total_products = "кол-во: <font style='font-size:14px;margin-left:5px;line-height:0px;'>".$am."</font>";


    //$total_price = $CURRENCY_DISPLAY->getFullValue( $total );
    //fix!!!
    $_SESSION['totalprice']= $total;
    $total_price = "всего: <font style='font-size:14px;margin-left:7px;line-height:0px;'>".$CURRENCY_DISPLAY->getFullValue( $total )."</font>";
}
