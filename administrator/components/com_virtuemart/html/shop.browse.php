...
//		}
		...
		if (USE_AS_CATALOGUE != '1' && $product_price != ""
            ...
            $tpl->set( 'product_name', $db_browse->f('product_name') );
        ...
        $products[$i]['prod_id'] = $db_browse->f("product_id");

//fix!!!
        $pid=$products[$i]['prod_id'];
        if (!empty($pid)) {
            $db_attr = new ps_DB;
            $qa="SELECT attribute_value FROM `jos_vm_product_attribute` WHERE `attribute_name`='download' and `product_id`=".$pid;
            $db_attr->query($qa);
            $ii=0;
