...
//fix!!!
if($orderby=='rate')
    $join_array = array( 'LEFT JOIN `#__{vm}_product_price` ON `#__{vm}_product`.`product_id` = `#__{vm}_product_price`.`product_id` LEFT JOIN `jos_content_extravote` ON `jos_vm_product`.`product_id` = `jos_content_extravote`.`content_id` ' );
else
...
switch( $orderby ) {
    ...
    case 'rate':
        $orderbyField = '(jos_content_extravote.rating_sum/jos_content_extravote.rating_count)'; break;
...
//print_r($_POST);
//if(!$_POST['Search']) fix!!!
if((!$_POST['Search'])&&(!$_GET['getsearch']))
{
...
// Joomla! 1.5 supports listing "All" items, which meanss $limit == 0
