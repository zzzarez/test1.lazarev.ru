...
class PaymentMethodsStatistic extends Statistic
{
	/**
	...
	{		
		...
		$this->data =& $this->model->getPaymentMethodsData();
	}
	...
	function renderXmlData (){	
		...
		foreach ($this->data as $set) {
			$label = $this->shortenLabel(ucfirst($set->payment));
			$xml .= "<set label='".$label."' value='$set->count' hoverText='".ucfirst($set->payment).": $set->count' />";
		}
	...
	function renderXmlRevenueData (){	
		...
		foreach ($this->data as $set) {
			$label = $this->shortenLabel(ucfirst($set->payment));
			$xml .= "<set label='".$label."' value='$set->revenue' hoverText='".ucfirst($set->payment).": ".VmreportsHelper::numberFormat($set->revenue, VMREPORTS_DECIMAL_PRECISION, '.', ' ')." ".VMREPORTS_CURRENCY."' />";
		}
