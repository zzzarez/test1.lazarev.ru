var MooPrompt = box = new Class({
	...
	initialize: function(title, content, options){
        ...
        this.hdrwrapper = new Element('div').setProperty('class', 'cbWrapper').injectInside(this.box);
        this.header = new Element('span').setProperty('class', 'cbTxt').appendText('Вы добавили ').injectInside(this.hdrwrapper);
        this.aaa=new Element('span').setProperty('class', 'cbHeader').appendText(this.title).injectInside(this.hdrwrapper);

        //this.header.injectInside(this.header);
		...
		//fix!!! for (var i = 1; i <= this.options.buttons; i++) {
        for (var i = this.options.buttons; i >= 1; i--) {
