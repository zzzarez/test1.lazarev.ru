...
//fix!! echo '<span class="conf_warn">'. $VM_LANG->_($varname) . '</span>';
echo '<span class="conf_warn">Пожалуйста, проверьте данные и подтвердите заказ. Если Вам необходимо внести изменения в заказ, то вернитесь обратно в <a style="font-size: 16px;" href="/index.php?page=shop.cart&option=com_virtuemart&Itemid=1">"Корзину"</a></span>';
$db = new ps_DB();
...
<div align="left" style="visibility: hidden;height: 0px;">
    <?php //echo $VM_LANG->_('PHPSHOP_CHECKOUT_CUSTOMER_NOTE') ?>
    <textarea style="visibility: hidden;" title="<?php echo $VM_LANG->_('PHPSHOP_CHECKOUT_CUSTOMER_NOTE') ?>" cols="0" rows="0" name="customer_note"></textarea>
    <br />
    <?php
...
if( VM_ONCHECKOUT_SHOW_LEGALINFO == '1' ) {
    ...
    ?>
<div align="center">
<input type="submit" onclick="return( submit_order( this.form ) );" class="conf_button" style="width: 190px;height: 35px;font-size: 18px" name="formSubmit" value="<?php echo $VM_LANG->_('PHPSHOP_ORDER_CONFIRM_MNU') ?>" />
</div>
<?php
if(  PSHOP_AGREE_TO_TOS_ONORDER == '1' ) {
