...
// SO LEAVE THIS IN HERE!
    ...
    if ($children != "multi") {
       //echo "_".$this->get_cfg( 'useAjaxCartActions', 1 );
        ?>
    <form style="color:black;" action="<?php echo $mm_action_url ?>index.php?page=shop.cart&option=com_virtuemart" method="post" name="addtocart" id="<?php echo uniqid('addtocart_') ?>" class="addtocart_form"
            
        <?php  //if( $this->get_cfg( 'useAjaxCartActions', 1 )) {
            $pr_price=str_replace(" ", "",preg_replace("/\D/","",strip_tags($product_price)));
            ?>
onsubmit='handleAddToCart( this.id , "<?php echo $product_name; ?>","<?php echo $pr_price ; ?>");var sm=document.getElementById("sbm_btn<?php echo $product_id; ?>");sm.value="В корзине";sm.disabled=true; return false;'>
            <?php// } ?>
...
<!--            <div style="float: right;vertical-align: middle;"> -->
                ...
                ?>
                <input type="submit" id="sbm_btn<?php echo $product_id ?>" class="<?php echo $button_cls ?>" value="<?php echo $button_lbl ?>" title="<?php echo $button_lbl ?>" />
<!--            </div>-->
            <?php  } ?>
