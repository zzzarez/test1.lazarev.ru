...
foreach( $products as $product ) { 		
    ...
    <div class="vmCartChild<?php echo $cls_suffix." ".$product['bgcolor'].$cls_suffix ?>">
        <form action="<?php echo $mm_action_url ?>index.php" method="post" name="addtocart" id="addtocart<?php echo $product['product_id'] ?>" class="addtocart_form" onsubmit="handleAddToCart( this.id );var sm=document.getElementById('sbm_btn3697');sm.value='В корзине';sm.disabled=true;return false;">
            <div class="vmCartChildElement<?php echo $cls_suffix ?>">
                ...
                if (USE_AS_CATALOGUE != '1'  && $product_price != "" && !stristr( $product_price, $VM_LANG->_('PHPSHOP_PRODUCT_CALL'))) { ?>
                    ...
                    <input type="submit" id='sbm_btn3697' class="addtocart_button" value="<?php echo $VM_LANG->_('PHPSHOP_CART_ADD_TO') ?>" title="<?php echo $VM_LANG->_('PHPSHOP_CART_ADD_TO') ?>" /></span>
                <?php } 
                // Output Quantity Box 
