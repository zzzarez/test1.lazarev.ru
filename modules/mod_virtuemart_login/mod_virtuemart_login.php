...
//    return (trim($buffer));
//}
//ob_start(); //fix!!! stupid fucking fix
...
// Joomla 1.5
    ...
    if( $redirect == 'home' ) {
        ...
        $uri = JFactory::getURI( $default->link.'&failure=true&Itemid='.$default->id );
        $url = $uri->toString(array('path', 'query', 'fragment'));
...
//{
   // $style_fix="style='margin-top: -1.2em;'";
//}
?>
...
<!--            <a href="--><?php //$sess->purl(SECUREURL . "index.php?page=shop.downloads", true);?><!--">--><?php //echo $VM_LANG->_('PHPSHOP_DOWNLOADS_TITLE') ?><!--</a>-->
    ...
    <form action="<?php echo $action ?>" method="post" name="login" id="login">
        ...
        <?php
        if(( $_GET['failure']=='true')&&($_GET['logout']!='true'))
                {
                    echo "<b style='color: red;font-size: 14px;'>Неверный e-mail <br>или пароль!!!</b>";
                }
        //fix!!!
        ?>
...
<!--        </div>-->
    ...
    $oc=ob_get_contents(); //fix!!! stupid fucking fix
    ob_end_clean();
    //echo "---".substr($oc,0,3)."---";
    if($oc[0]!="<")
    {
    ...
    //echo $oc; document.getElementById('login_form').outerHTML
